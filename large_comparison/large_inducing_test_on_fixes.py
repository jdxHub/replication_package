...
BATCH_SIZE = 64
EPOCHS = 30 #30  # This should be at least 10 for convergence
SEQ_LENGTH = 100
...
PATH_CSV = '../datasets/large_dataset.json'
RESERVED_TOKENS = ["[PAD]", "[UNK]", "[START]", "[END]"]

MODEL_DIR_NAME = "../models/"
MODEL_NAME = 'large_vulnerability_inducing'
CLASSIFIER_NAME = 'rf_classifier_large_vulnerability_inducing.pkl'

# TODO: TROVON model paths
TROVON_PATH_CSV = '/content/replication_package/datasets/small_dataset.json' #TODO: TROVON dataset
...
def preprocess_json_dataset(json):
    ...
    for project in df_csv['abstract']:
      for vul in project['pre_vulnerability_inducing_vulnerability_inducing_diff']:
        ...
        functions.append((vul, 1))
      for vul in project['pre_vulnerability_inducing_vulnerability_inducing_same']:
        functions.append((vul, 0))

    functions = pd.DataFrame(data=functions, columns=['abs_seq', 'label'])
    return functions

def get_sequence_pairs(path_csv, trovon_path_csv):
    df_csv = pd.read_csv(trovon_path_csv)
    test_pairs = []
    total = 0
    positives = 0
    vocab = []
    ...
    #training pairs
    df_csv = preprocess_json_dataset(path_csv)
    text_pairs = []
    for index_df_csv in df_csv.index:
        abs_seq = df_csv['abs_seq'][index_df_csv]['before']
        inp_seq = abs_seq.replace("\"","")
        imitates_vuln = df_csv['label'][index_df_csv]
        label = 0
        if imitates_vuln == True:
            out_seq = inp_seq + " " + KEYWORD_POSITIVE
            positives = positives + 1
            ...
            out_seq = inp_seq + " " + KEYWORD_NEGATIVE
        test_pairs.append((inp_seq, out_seq, label))
        total = total + 1
        arr_inp_seq = inp_seq.split()
        for word in arr_inp_seq:
            if word not in vocab:
    ...
    print()
    return text_pairs, test_pairs, SEQ_LENGTH, INP_VOCAB_SIZE, OUT_VOCAB_SIZE
...
def train_model(epochs, transformer, train_ds, val_ds, classifier_data):
    ...
    for epoch in range(1, epochs + 1):
        ...
        transformer.save(model_dir_name + model_name + '_epoch_%d' % epoch)
        transformer = keras.models.load_model(model_dir_name + model_name + '_epoch_%d' % epoch)
        print()
        ...
        hist_data.append({**history.history, 'epoch': epoch})
...
text_pairs, test_pairs, MAX_SEQUENCE_LENGTH, INP_VOCAB_SIZE, OUT_VOCAB_SIZE = get_sequence_pairs(TROVON_PATH_CSV, PATH_CSV)

train_pairs, val_pairs = get_test_val_test_pairs(text_pairs)
...
lst_transformer_predictions, str_transformer_result, lst_classifier_predictions, str_classifier_result, rowMetrics = get_all_models_performances(NUM_BEAMS, test_pairs, inp_tokenizer, inp_tokenizer, transformer, clf)
# save metrics
hist_data = [rowMetrics]
df = pd.DataFrame(hist_data)
df.to_csv("results_inducing.csv")

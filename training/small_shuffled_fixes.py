...
BATCH_SIZE = 64
EPOCHS = 40 #30  # This should be at least 10 for convergence
...
BASE_MODEL_NAME = 'small_shuffled_fixes'
BASE_CLASSIFIER_NAME = 'rf_classifier_small_shuffled_fixes.pkl'
...
def split_samples(dataset):
    num_val_samples = int(0.10 * len(dataset))
...
def preprocess_json_dataset(json):
    ...
    for project in df_csv['abstract']:
      for pre_patch in project['pre_patch_patch_diff']:
        positives.append((pre_patch, 1))
      for pre_patch in project['pre_patch_patch_same']:
        negatives.append((pre_patch, 0))
    ...
    train_pairs, val_pairs = tuple(map(lambda i, j: i + j, split_samples(positives), split_samples(negatives)))
    res = train_pairs + val_pairs
    print(len(res))
    tests = pd.DataFrame(data=tests, columns=['abs_seq', 'label'])
    return pd.DataFrame(data=res, columns=['abs_seq', 'label']), tests

def get_sequence_pairs(path_csv):
    df_csv, df_tests = preprocess_json_dataset(path_csv)
    text_pairs = []
    tests_pairs = []
    total = 0
    positives = 0
    ...
    for index_df_csv in df_csv.index:
      ...
      label = df_csv['label'][index_df_csv]
      if label == 1:
    ...
    print()
    return text_pairs, MAX_SEQ_LENGTH, INP_VOCAB_SIZE, OUT_VOCAB_SIZE, tests_pairs
...
def get_csv_merged_with_predictions (path_csv, lst_transformer_predictions, str_transformer_result, lst_classifier_predictions, str_classifier_result):
    ...
    df_csv, _ = preprocess_json_dataset(path_csv)
    row_index = 0
    for index_df_csv in df_csv.index:
      row_label = df_csv['label'][index_df_csv]
      row_vul_id = "none"
      row_cve_id = "none"
      row_class = df_csv['abs_seq'][index_df_csv]['filename']
      row_mut_id = 0
...
text_pairs, MAX_SEQUENCE_LENGTH, INP_VOCAB_SIZE, OUT_VOCAB_SIZE, test_pairs = get_sequence_pairs(PATH_CSV)
